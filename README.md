...
| **Yogesh Patel** | Forecasting models (`forecasters.py`): `Forecaster` base class, random-walk and EWMA subclasses, `@require_fitted` decorator; calibration metrics (`calibration.py`): CRPS, coverage, reliability diagrams; backtesting engine (`backtest.py`); edge-case test suite (`test_edge_cases.py`, 6 cases); results and discussion, known limitations, and future improvements sections of the README; repository setup and documentation consistency fixes. |
| **Olin Dsouza** | Main program (`main.ipynb`): full pipeline integration and analysis; forecasting models (`forecasters.py`): `Forecaster` base class, random-walk and EWMA subclasses, `@require_fitted` decorator; calibration metrics (`calibration.py`) and backtesting engine (`backtest.py`); test suite (`test_toolkit.py`, 13 cases); portfolio operator overloads (`__add__`, `__eq__`); `data_updater.py` hardening (specific exception handling, API-key security); documentation (`README.md`), dependency management (`requirements.txt`, `.gitignore`). |
